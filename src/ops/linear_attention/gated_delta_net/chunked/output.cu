...
cudaError_t launch_output(const chunk_output_config& cfg) {
    ...
    // Keep at most one resident wave on the current device and distribute chunks evenly
    // across it. Small grids retain one logical job per CTA.
    const std::int64_t target_ctas    = static_cast<std::int64_t>(current_device_sm_count()) *
                               kCtasPerSm;
