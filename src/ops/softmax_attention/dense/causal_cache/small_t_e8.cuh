...
// ninfer::ops - split-KV causal small-T attention, E8 KV-cache partial kernel (RK4V4E8, RK2V4E8).
//
//   * QK runs on native m16n8k32.s8 tensor cores. Q and the E8 codec both H64-rotate per
//     64-dimension group; K stays packed-4-bit (RK4V4E8) or E8 root-cylinder (RK2V4E8) in the
//     U8 cache and is decoded once into the INT8 QK arena. The int32 MMA output is rescaled per
//     64-group by qs[row,g]*ks[key,g], the same as the INT8 path.
//   * PV stays bf16: V is stored packed-4-bit and decoded straight to a bf16 tile (no int8
//     intermediate), matching the INT8 path's V staging. V (and the PV output) is H64-rotated;
//     the launcher applies the inverse rotation to the attention result.
//   * All keys (history AND the current/diagonal tokens) are read from the quantized cache; the
//     fused append writes the new tokens in E8 format first and a __syncthreads orders the
//     in-block readback. No from_new special-casing.
//
...
#include "ops/kv_cache/e8_kv_device.cuh"
...
// Decode one 8-byte packed-4-bit key chunk (16 rotated dimensions inside one G64 group) into the
// swizzled INT8 QK arena.
__device__ __forceinline__ void causal_small_t_e8_decode_packed_k16(const std::uint8_t* raw8,
                                                                  std::int8_t* k_tile, int row, int d,
                                                                  int d_b16_stride) {
...
// Decode one 16-byte E8 root key chunk (one 64-dimension group: eight 8-dimension blocks) into the
// swizzled INT8 QK arena. Each block stores a consecutive (root, rad) byte pair, matching
// kv_cache_append_e8_group.
template <bool E8Root>
__device__ __forceinline__ void causal_small_t_e8_decode_key_group(const std::uint8_t* raw16,
                                                                 std::int8_t* k_tile, int row, int d,
                                                                 int d_b16_stride) {
    if constexpr (E8Root) {
#pragma unroll
...
template <typename Geometry, int TokenTile, int WarpsPerCta, int MinBlocksPerSm, int KeyBlock,
          bool DynamicArena, bool MultiBatch, bool Masked, bool E8Root, typename CacheInput>
__launch_bounds__(WarpsPerCta * 32, MinBlocksPerSm) __global__
    void causal_attention_small_t_e8_tiled_kernel(
        const __nv_bfloat16* q, CacheInput input, const std::int32_t* pos, std::uint8_t* cache_k_i8,
        std::uint8_t* cache_v_i8, __half* cache_k_scale, __half* cache_v_scale,
    ...
    // E8: V decodes straight to bf16 (no int8 intermediate), freeing the region that held v_i8.
    // The raw E8 K/V code arenas follow; RK4V4E8 K is D/2 per row, RK2V4E8 K is D/4 per row, V is
    // always D/2 per row. This keeps the total at (RK4) or below (RK2) the INT8 4*Bc*D budget.
    constexpr int KE8Bytes = E8Root ? (kCausalHeadDim / 4) : (kCausalHeadDim / 2);
    __nv_bfloat16* v_bf16 = reinterpret_cast<__nv_bfloat16*>(r_s + Bc * D);
    std::uint8_t* k_raw   = reinterpret_cast<std::uint8_t*>(r_s + 3 * Bc * D);
    std::uint8_t* v_raw   = k_raw + Bc * KE8Bytes;
    ...
    if constexpr (CacheInput::writes_cache) {
        ...
        // 4-bit lattice plane (RK4V4E8) or the E8 root-cylinder plane (RK2V4E8) for K and the
        // packed 4-bit plane for V, with per-group FP16 scales. E8Lattice is the complement of
        // E8Root because the two modes are mutually exclusive.
        for (int pair = warp; pair < valid_tokens * Groups; pair += Wc) {
            ...
            kv_cache_append_e8_group<Geometry, !E8Root, E8Root>(
                input.k, input.v, kv_head, grp, token, physical_page, page_offset, cache_k_i8,
                cache_v_i8, cache_k_scale, cache_v_scale, lane);
        }
        __syncthreads();
    }
...
#pragma unroll
        ...
        // E8 Q is H64-rotated per 64-dimension group (matching the H64-rotated E8 K cache).
#pragma unroll
...
#pragma unroll 1
        if constexpr (E8Root) {
            for (int chunk = tid; chunk < Bc * Groups; chunk += Threads) {
                ...
                const int key    = tile_k0 + key_l;
                std::uint8_t* kd = &k_raw[key_l * KE8Bytes + grp * (D / 4) / Groups];
                if (key >= split_start && key < split_end) {
                    const std::int64_t off = kv_cache_int8_quant_e8_code_index<Geometry>(
                        physical_page, kv_head, grp * (D / 4) / Groups, key & kPagedKVPageMask);
                    ninfer::ops::cp_async<16>(kd, &cache_k_i8[off]);
                } else {
            ...
            for (int chunk = tid; chunk < Bc * (D / 16); chunk += Threads) {
                ...
                const int d      = dc * 16;
                const int key    = tile_k0 + key_l;
                std::uint8_t* kd = &k_raw[key_l * KE8Bytes + d / 2];
                if (key >= split_start && key < split_end) {
                    const std::int64_t off = kv_cache_int8_quant_i4_code_index<Geometry>(
                        physical_page, kv_head, d / 2, key & kPagedKVPageMask);
                    ninfer::ops::cp_async<8>(kd, &cache_k_i8[off]);
                } else {
                    store_vec(kd, make_int2(0, 0));
                }
            }
        }
        // Packed value codes: 8 bytes per (key row, 16 rotated dimensions).
#pragma unroll 1
        for (int chunk = tid; chunk < Bc * (D / 16); chunk += Threads) {
            const int key_l  = chunk / (D / 16);
            const int dc     = chunk - key_l * (D / 16);
            ...
            if (key >= split_start && key < split_end) {
                const std::int64_t off = kv_cache_int8_quant_i4_code_index<Geometry>(
                    physical_page, kv_head, d / 2, key & kPagedKVPageMask);
                ninfer::ops::cp_async<8>(vd, &cache_v_i8[off]);
            } else {
    ...
    // Cooperative decode of the staged raw key codes into the INT8 QK arena. Must run after the
    // tile's cp.async wait and before the producer QK consumes k_i8.
    auto decode_k = [&]() {
        if constexpr (E8Root) {
#pragma unroll 1
            for (int chunk = tid; chunk < Bc * Groups; chunk += Threads) {
                const int key_l = chunk / Groups;
                const int grp   = chunk - key_l * Groups;
                causal_small_t_e8_decode_key_group<E8Root>(
                    &k_raw[key_l * KE8Bytes + grp * (D / 4) / Groups], k_i8, key_l,
                    grp * (D / Groups), DB16);
            }
        } else {
#pragma unroll 1
            for (int chunk = tid; chunk < Bc * (D / 16); chunk += Threads) {
                ...
                const int dc    = chunk - key_l * (D / 16);
                const int d     = dc * 16;
                causal_small_t_e8_decode_packed_k16(&k_raw[key_l * KE8Bytes + d / 2], k_i8, key_l,
                                                    d, DB16);
            }
        }
    };
