// Coverage for the production E8 root codec in src/ops/kv_cache/e8_root_codec.cuh.
//
// Ported from the ninfer-4090 fork (tests/ops/test_e8_root_codec.cu) unchanged apart from
// the codec's path, which moved from src/ops/kernel/ to src/ops/kv_cache/ in this tree.
// The fork also carries an independent reimplementation of the E8 mathematics under
// tools/test_kv/; that testbed never includes this header, so it cannot catch a regression
// in the code the rk2v4-e8 fill path actually compiles. This suite drives the shipped
// functions directly. It covers the key encoder only - see the note at the end of this
// comment.
//
...
//
// Scope: this suite ends at the cylinder encoder. It does not touch the E8 cache planes,
// the fused/standalone appends, or the attention kernels that read them - the fork had no
// op test at that level either, which is why two E8 attention defects reached the engine
// during this port. That gap is covered by ninfer_softmax_attention_test's E8 entries.
